...
- [ ] Task: Update `bin/platforms/index.ts`
    - [ ] Update `copilot` path to `.github/agents`.
    - [ ] Add `copilotPrompts` (path: `.github/prompts`, ext: `.prompt.md`).
    - [ ] Add `copilotSkills` (path: `~/.copilot/skills`, pattern: `<name>/SKILL.md`).
    - [ ] **Verify**: Run `pnpm dev -- --dry-run` to see proposed paths.
